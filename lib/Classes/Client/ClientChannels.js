...
module.exports = class ClientChannels {
    ...
    async find(id) {
        ...
        if (channel.type || channel.type === 0) {
            ...
            else if (channel.type === 3) return new Channel(channel, this.client) // should not be possible (self bots)
            else if (channel.type === 4) return new Category(channel, this.client)
            else if (channel.type === 5) return new News(channel, this.client)
            else if (channel.type === 6) return new Store(channel, this.client)
            else if (channel.type === 13) return new Stage(channel, this.client)
        }
