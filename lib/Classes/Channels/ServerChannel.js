...
class ServerChannel extends Channel {
    ...
    async name(name, reason) {
        ...
        let channel = methods.name(name, this.id, this.client.token, reason)
        return channel
    }
    async parent(category, reason) {
        channel = await methods.parent(category, this.id, this.client.token, reason)
        return channel
    }
    async position(position, reason) {
        if (typeof position !== 'number') throw new Error('.position() parameter is not a number')
        channel = await methods.position(position, this.id, this.client.token, reason)
        return channel
    }
    async newInvite() {
        invite = await methods.position(this.id, this.client.token, reason)
    }
    async editPerm() {
    }
    async delete(){
