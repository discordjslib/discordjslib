...
module.exports = class Message {
  constructor(message, channel, client) {
    ...
    if (message.member)
      this.guild =
        message.member.guild || channel.guild || message.guild || null;
    this.id = message.id;
    this.client = client;
  ...
  async react(emoji) {
    let message = await methods.react(
      this.id,
      this.channel.id,
      emoji,
      this.client.token
  ...
  async reply(content, embed) {
    if (typeof content === "string" && !embed) {
      const message = await methods.reply(
        this.id,
        this.channel.id,
        content,
        this.client.token
      ...
      const message = await methods.replyEmbed(
        this.id,
        this.channel.id,
        content,
        this.client.token
      ...
      const message = await methods.messageAndEmbed(
        this.id,
        this.channel.id,
        content,
        this.client.token
  ...
  async edit(content) {
    let message = await methods.edit(
      this.id,
      this.channel.id,
      String(content),
      this.client.token
