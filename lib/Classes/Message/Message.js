...
module.exports = class Message {
    ...
    async react(emoji) {
        try {
            ...
            const reaction = {}
            return new Reaction(reaction, this, this.client)
        } catch (err) {
        }
    }
    async reply(content) {
    ...
    // async send(content){
    //     if (typeof content === 'string') {
    //         const message = await methods.send(this.channel.id, content, this.client.token)
    //         console.log(message)
    //         if (message.code) throw new Error(`${message.message}`)
    //         return new Message(message, this.channel, this.client)
    //     } else if (typeof content === 'object') {
    //         const message = await methods.sendEmbed(this.channel.id, content, this.client.token)
    //         if (message.code) throw new Error(`${message.message}`)
    //         return new Message(message, this.channel, this.client)
    //     } else if (typeof content === 'function') {
    //         if (content.toObject()) {
    //             const message = await methods.sendEmbed(this.channel.id, content.toObject(), this.client.token)
    //             if (message.code) throw new Error(`${message.message}`)
    //             return new Message(message, this.channel, this.client)
    //         }
    //         else throw new Error('Invalid .reply() paramater')
    ...
    async delete(){
        let message = await methods.delete(this.id, this.channel.id, this.client.token)
    }
}